...
@section('title','Admin | Manage City')
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i> Manage City
                </h4>
                <a href="{{ route('city.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($cities as $city)
                            <tr>
                                <td>{{ isset($city->city_name) ? $city->city_name : '-' }}</td>
                                <td>{{ isset($city->cntry->country_name) ? $city->cntry->country_name : '-' }}</td>
                                @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin')
                                <td class="text-center">

                                    <a class="btn btn-success btn-sm" href="{{ route('city.edit',$city->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form class="deleteform d-inline-block" action="{{route('city.destroy',$city->id)}}" method="post">
                                        @csrf
                                        @method('delete')
