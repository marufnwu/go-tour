...
@section('title','Admin | Manage Airport')
    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Manage Airport</li>
            </ol>
        </nav>
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i> Manage Airport
                </h4>
                <a href="{{ route('airport.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                    <i class="fa fa-plus"></i>&nbsp; Add Airport </a>
            </div>
            <div class="card-body">
                <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        <thead>
                        <tr>
                            <th class="align-text-top">Airport Name</th>
                        ...
                        @foreach($airports as $airport)
                            <tr>
                                ...
                                <td>{{ isset($airport->cty->city_name) ? $airport->cty->city_name : '-' }}</td>
                                <td>{{ isset($airport->cntry->country_name) ? $airport->cntry->country_name : '-' }}</td>
                                @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin')
                                <td class="text-center">

                                    <a class="btn btn-success btn-sm" href="{{ route('airport.edit',$airport->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form class="deleteform d-inline-block" action="{{route('airport.destroy',$airport->id)}}" method="post">
                                        @csrf
                                        @method('delete')
