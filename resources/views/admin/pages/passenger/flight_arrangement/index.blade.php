...
@section('title','Admin | Flight Arrangement')
    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Flight Arrangement</li>
            </ol>
        </nav>
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i> Flight Arrangement
                </h4>
                {{-- <a href="{{ route('flight_arrangement.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                    <i class="fa fa-plus"></i>&nbsp; Add Flight Arrangement</a> --}}
            </div>
            <div class="card-body">
                <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($flights as $flight)
                            <tr>
                                <td>{{ @$flight->city_depart->airport_name }}</td>
                                <td>{{ @$flight->city_arrival->airport_name }}</td>
                                <td>{{ date_format(date_create($flight->departure_date), 'd M Y h:i A') }}</td>
                                <td>{{ date_format(date_create($flight->arrival_date_time), 'd M Y h:i A') }}</td>
                                <td>{{ $flight->ticket_pnr }}</td>
                                <td>{{ $flight->airline }}</td>
                                <td>{{ $flight->ticket_price }}</td>
                                
                                {{-- <td class="text-center">

                                    <a class="btn btn-success btn-sm" href="{{ route('flight_arrangement.edit',$flight->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form class="deleteform d-inline-block" action="{{route('flight_arrangement.destroy',$flight->id)}}" method="post">
                                        @csrf
                                        @method('delete')
