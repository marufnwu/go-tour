...
@section('title','Hotel | Hotel Reservation')
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                ...
                {{-- <a href="{{ route('hotel.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                  ...
                  <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                      ...
                      @foreach($data as $reservation)
                          <tr>
                              ...
                              <td>{{ date_format(date_create($reservation->from_date), 'd M Y h:i A') }}</td>
                              <td>{{ date_format(date_create($reservation->to_date), 'd M Y h:i A') }}</td>
                              <td>{{ $reservation->total_passenger }}</td>
                              <td>{{ $reservation->single_room }}</td>
                              <td>{{ $reservation->double_room }}</td>
                              <td>{{ $reservation->triple_room }}</td>
                              <td>{{ date_format(date_create($reservation->reservation_date), 'd M Y h:i A') }}</td>
                              <td>{{ date_format(date_create($reservation->confirmation_date), 'd M Y h:i A') }}</td>
                              {{-- @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin') --}}
                              <td class="text-center">

                                  <a class="btn btn-info btn-sm mb-1" href="{{ route('hotel_reservation.edit',$reservation->id) }}">
