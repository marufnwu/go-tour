...
@section('title','Admin | Edit Tour Leader Tour')
    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('tourleader_tour.index') }}">Manage Tour Leader Tour</a></li>
                <li class="breadcrumb-item active" aria-current="page">Edit Tour Leader Tour</li>
            </ol>
        </nav>
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i>  Edit Tour Leader Tour
                </h4>

            </div>
            <form id="ajaxForm" class="" action="{{route('tourleader_tour.update',$tours->id)}}" method="POST">
            @csrf
            @method('put')
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-4">
                        <div class="form-group">
                            ...
                            <input id="from" type="number" min="0" class="form-control form-margin-bottom" name="tour_code" value="{{ $tours->tour_code }}"  placeholder="Tour code...">
                            @error('tour_code')
                    ...
                    <div class="form-group">
                        ...
                        <input id="from" type="number" min="0" class="form-control form-margin-bottom" name="tour_cost" value="{{ $tours->tour_cost }}"  placeholder="Tour cost...">
                        @error('tour_cost')
                            <span class="input-error">{{$message}}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control form-margin-bottom" name="language">
                              ...
                              @foreach ($languages as $language)
                                <option value="{{ $language }}" {{ $tours->language == $language ? 'selected':''}}>{{ $language }}</option>
                              @endforeach
                            </select>
                        @error('language')
                            <span class="input-error">{{$message}}</span>
                ...
                <div class="row">
                    <div class="col-lg-4">
                        <div class="form-group">
                            ...
                            @foreach ($tourleaders as $tourleaders)
                                <option value="{{ $tourleaders->id }}" {{ $tours->tourleader_id  == $tourleaders->id ? 'selected' : '' }}>{{ $tourleaders->tlfirst_n .' '. $tourleaders->ti_l_name}}</option>
                            @endforeach
                            </select>
                            @error('tourleader_id')
                    ...
                    <div class="col-lg-4">
                        <div class="form-group">
                            ...
                            <select class="form-control form-margin-bottom" name="gti_id">
                                <option value="">Select GTI</option>
                                @foreach ($gtis as $gti)
                                <option value="{{ $gti->id }}" {{ $tours->gti_id  == $gti->id ? 'selected' : '' }}>{{ $gti->gti_name}}</option>
                                @endforeach
                            </select>
                            @error('gti_id')
                    ...
                    <div class="col-lg-4">
                        <div class="form-group">
                            ...
                            <input id="from" type="number" min="0" class="form-control form-margin-bottom" name="passengers" value="{{$tours->passengers}}"  placeholder="Approximate passengers...">
                            @error('passengers')
                            <span class="input-error">{{$message}}</span>
                            @enderror 
                        </div>
                ...
                <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input id="from" type="text" class="form-control form-margin-bottom" name="tour_name" value="{{ $tours->tour_name }}"  placeholder="Tour name...">
                                @error('tour_name')
                                    <span class="input-error">{{$message}}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input id="from" type="date" class="form-control form-margin-bottom" name="athens_departure_date" value="{{ $tours->athens_departure_date }}">
                  ...
                  <div class="col-12">
                      <div class="row">
                          ...
                          <div class="col-xl-4 col-lg-4 col-md-6 col-12">
                              <button id="submitBtn" type="submit" class="btn btn-primary btn-lg btn-block"><i class="fa fa-check-circle" aria-hidden="true"></i> Update</button>
                          </div>
                      </div>
                  </div>
              </div>
