...
@section('title','Admin | Flight Reservation')
    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Flight Reservation</li>
            </ol>
        </nav>
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i> Flight Reservation
                </h4>
                <a href="{{ route('flight_reservation.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($flights as $flight)
                            <tr>
                                <td>{{ isset($flight->tour->tour_name) ? $flight->tour->tour_name : '-' }}</td>
                                <td>{{ date_format(date_create($flight->departure_date), 'd M Y h:i A') }}</td>
                                <td>{{isset($flight->airportD->airport_name)?$flight->airportD->airport_name:'-'}}</td>
                                <td>{{isset($flight->airportA->airport_name)?$flight->airportA->airport_name:'-'}}</td>
                                @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin')
                                <td class="text-center">
                                    <a class="btn btn-success btn-sm" href="{{ route('flight_reservation.edit',$flight->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form class="deleteform d-inline-block" action="{{route('flight_reservation.destroy',$flight->id)}}" method="post">
                                        @csrf
                                        @method('delete')
