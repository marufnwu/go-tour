...
@section('title','Admin | Supplier Type')
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i> Supplier Type
                </h4>
                <a href="{{ route('type_supplier.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                    <i class="fa fa-plus"></i>&nbsp; Add Supplier Type </a>
            </div>
            <div class="card-body">
                <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($suppliertypes as $suppliertype)
                            <tr>
                                <td>{{ $suppliertype->supplier_type }}</td>
                                <td>{{ $suppliertype->user_type }}</td>
                                @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin')
                                <td class="text-center">

                                    <a class="btn btn-success btn-sm" href="{{ route('type_supplier.edit',$suppliertype->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form class="deleteform d-inline-block" action="{{route('type_supplier.destroy',$suppliertype->id)}}" method="post">
                                        @csrf
                                        @method('delete')
