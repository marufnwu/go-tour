    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> Hotel price</li>
            </ol>
        </nav>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($fees as $fee)
                            <tr>
                                <td>{{ isset($fee->hotel->hotel_name) ? $fee->hotel->hotel_name : '-' }}</td>
                                <td>{{ date_format(date_create($fee->from_date), 'd M Y h:i A') }}</td>
                                <td>{{ date_format(date_create($fee->to_date), 'd M Y h:i A') }}</td>
                                <td>{{ isset($fee->accom->accommodation_type_name) ? $fee->accom->accommodation_type_name : '-' }}</td>
                                <td>{{ $fee->price }}</td>
                                @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin')
