...
@section('title','Admin | Guide Reservation')
    ...
    <div class="container-fluid">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Guide Reservation</li>
            </ol>
        </nav>
        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i> Guide Reservation
                </h4>
                <a href="{{ route('reserve_guide.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                    <i class="fa fa-plus"></i>&nbsp; Add Guide Reservation</a>
            </div>
            <div class="card-body">
                <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        ...
                        @foreach($reservations as $reservation)
                            <tr>
                                <td>{{ @$reservation->guide->guide_first_n .' '. @$reservation->guide->guide_l_name }}</td>
                                ...
                                <td>{{ date_format(date_create($reservation->confirm_date), 'd M Y') }}</td>
                                @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin')
                                <td class="text-center">

                                    <a class="btn btn-success btn-sm" href="{{ route('reserve_guide.edit',$reservation->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form class="deleteform d-inline-block" action="{{route('reserve_guide.destroy',$reservation->id)}}" method="post">
                                        @csrf
                                        @method('delete')
