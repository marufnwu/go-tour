        ...
        <div class="card shadow mb-4">
            <div class="card-header bg-default py-2 d-flex justify-content-between align-items-center">
                <h4 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-user-circle"></i> Guides
                </h4>
                <a href="{{ route('guide.create') }}" class="d-none d-sm-inline-block btn btn-primary shadow-sm btn-rounded" >
                    <i class="fa fa-plus"></i>&nbsp; Add Guide </a>
            </div>
            <div class="card-body">
                <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm js-dt" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <th class="align-text-top">Fees / Day</th>
                        ...
                        @foreach($guides as $guide)
                            <tr>
                                <td>
                                    <img src="{{ asset('/uploads/guide-profile/'.$guide->image) }}" height="60" class="rounded" alt="Profile image">
                                </td>
                                <td>{{ $guide->guide_first_n }} {{ $guide->guide_m_name }} {{ $guide->guide_l_name }}</td>
                                <td>{{ $guide->guide_address }}</td>
                                <td>{{ $guide->guide_city }}</td>
                                <td>{{ $guide->guide_country }}</td>
                                <td>{{ $guide->guide_phone }}</td>
                                <td>{{ $guide->guide_email }}</td>
                                <td>{{ $guide->guide_fees_per_day }}</td>

                                @if (auth()->user()->type=='CHCP' || auth()->user()->type=='Admin')
                                <td class="text-center">

                                    <a class="btn btn-success btn-sm" href="{{ route('guide.edit',$guide->id) }}">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form class="deleteform d-inline-block" action="{{route('guide.destroy',$guide->id)}}" method="post">
                                        @csrf
                                        @method('delete')
